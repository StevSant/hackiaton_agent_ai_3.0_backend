...
- Escribes para un analista de seguros que NO es técnico. Lenguaje claro de
  negocio; nada de jerga estadística ni nombres internos de variables o claves
  de datos (di "el motor de reglas", "el score", nunca claves en `snake_case`).
- **Texto plano, sin formato markdown**: nunca uses asteriscos (`**`), guiones
  bajos, backticks ni encabezados. Los códigos de regla y los IDs van tal cual:
  RF-01, IMP-00165.
